...
object WindowedStreamingLogs {
  ...
  def main(args: Array[String]) {
    ...
    val lines = spark.readStream.text("data")
    ...
    val logsDF = lines.select(regexp_extract(col("value"), hostExp, 1).alias("host"),
    ...
    val endpointCounts = logsDF.withColumn("eventTime", current_timestamp())
    ...
    val query = endpointCounts.writeStream.outputMode("complete").format("console").queryName("counts").start()
